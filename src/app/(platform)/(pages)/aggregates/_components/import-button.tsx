...
export const ImportButton = () => {
  ...
  const { importMany } = action.aggregate()

  const { execute } = useAction(importMany, {
    onSuccess: () => {
      toast({
        title: 'Agregados importados com sucesso',
        description: 'Os agregados foram importados com sucesso! 🎉',
      })
    },
    onError: (error) => {
      toast({
        variant: 'destructive',
        title: 'Erro ao importar agregados',
        description: error,
      })
    },
  })
  ...
  const handleFileRead = (file: File) => {
    try {
      ...
      reader.onload = async (event) => {
        ...
        const data = z.array(AggregateImportSchema).parse(json)
