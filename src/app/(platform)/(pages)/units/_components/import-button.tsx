...
import { UnitImportSchema } from '@/actions/unit/schema'
...
export const ImportButton = () => {
  ...
  const { importMany } = action.unit()

  const { execute } = useAction(importMany, {
    onSuccess: () => {
      toast({
        title: 'Unidades importadas com sucesso',
        description: 'As unidades foram importadas com sucesso! 🎉',
      })
    },
    onError: (error) => {
      toast({
  ...
  const handleFileRead = (file: File) => {
    try {
      ...
      reader.onload = async (event) => {
        ...
        const data = z.array(UnitImportSchema).parse(json)
  ...
  const downloadTemplate = () => {
    ...
    link.href = '/import-templates/units.xlsx'
    link.download = 'Importação-Unidades.xlsx'
    link.click()
